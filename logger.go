...
func (l *Logger) LogHost(data *HostGPUData) {
	...
	if data.Status != "online" {
		fmt.Fprintf(f, "[%s] 异常: %s\n", ts, data.Error)
		f.Sync()
		return
	}

	sys := data.Sys
	fmt.Fprintf(f, "[%s] CPU=%.1f%% MEM=%.0f/%.0fMB(%.1f%%) LOAD=%.2f/%.2f/%.2f | %d GPUs\n",
		ts, sys.CPUUsage, sys.MemUsed, sys.MemTotal, sys.MemPercent,
		sys.Load1, sys.Load5, sys.Load15, len(data.GPUs))

	for _, gpu := range data.GPUs {
		fmt.Fprintf(f, "  GPU%d %s %.0f℃ %.0f%% %.0f/%.0fMiB(%.0f%%) %.1f/%.1fW",
